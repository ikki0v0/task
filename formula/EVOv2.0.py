...
def get_result():
    ...
    driver.find_element_by_xpath('//*[@id="btnCalculate"]').click()  # 点击计算
    # time.sleep(1)
    # 判断power是否在列表里
    d = driver.find_element_by_xpath('//*[@id="PnPred"]/table/tbody').text
    list = d.split()
    list = list[34:44]
    return list
...
# 读取对应公式网站的xpath
f = 'EVO'  # 确定计算的公式
xpath = pd.read_csv(web_path, header=0, index_col=0, encoding="utf-8")


# 打开网页
driver = webdriver.Chrome(executable_path=path)
url = "https://www.evoiolcalculator.com/calculator.aspx"
driver.get(url)
...
for i in tqdm(range(df.shape[0])):
    ...
    while power > float(list[len(list)-2]):
        val = val - 1
        driver.find_element_by_xpath('//*[@id="btnBack"]').click()  # 返回填写页面
        driver.find_element_by_xpath('//*[@id="txtRefraction"]').clear()
        driver.find_element_by_xpath('//*[@id="txtRefraction"]').send_keys(val)  # Refraction
        # 计算
        list = get_result()
    ...
    df.loc[i, 'EVO'] = refraction
    print("\n", df.iloc[i, :])

    # 初始化回原来的界面
    driver.find_element_by_xpath('//*[@id="btnBack"]').click()  # 返回填写页面
    time.sleep(1)
    driver.find_element_by_xpath('//*[@id="btnReset"]').click()  # 清空页面
